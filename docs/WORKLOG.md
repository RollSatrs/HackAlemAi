...
- **Ограничения:** Известный пробел evaluator (`average_waiting_time_sec` недосчитывает машины в очереди на конец симуляции, `backend/src/evaluator/evaluator.ts:110-136`, задокументирован в SDD-ledger Task 4) — не устранялся, только зафиксирован в `docs/HANDOFF.md`.
