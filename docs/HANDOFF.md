...
**Проблемы / блокеры**
- Известный пробел (задокументирован ранее, Task 4 SDD-ledger, не устранялся намеренно): `average_waiting_time_sec` в evaluator (`backend/src/evaluator/evaluator.ts:35-61`) недосчитывает машины, всё ещё стоящие в очереди на момент окончания симуляции — метрика смещена вниз симметрично для baseline И каждого кандидата в одном прогоне, поэтому относительное сравнение (выбор лучшей политики) остаётся корректным, но абсолютное значение среднего времени ожидания занижено. Не блокирует sandbox-демо; требует исправления перед любым production-использованием реальных цифр.
